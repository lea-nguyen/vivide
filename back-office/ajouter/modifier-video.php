...
?>
    ...
    $placeholder = "SELECT * FROM `videos` WHERE id_video=:id_video";
    $stmt2 = $db->prepare($placeholder);
    $stmt2->bindParam(':id_video', $_GET["id"], PDO::PARAM_INT);
    $stmt2->execute();
    $placeholder = $stmt2->fetch(PDO::FETCH_ASSOC);

    ?>
    <main>
        <a class="retour" href="../videos.php?order=0&by=id_video">Retour</a>
        <?php
        echo "<p>Vous souhaitez <a class='btn3' href='../actions/modifier-video.php?id={$placeholder["id_video"]}' >supprimer la vidéo</a>, cette action supprimera définitivement la vidéo</p>";
        ?>
